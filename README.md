...
**Deck templates:** `aula-capitulo`, `pitch-projeto`, `demo-tecnica` + any custom templates generated by `brand`.
**Themes:** `mira-dark`, `light-minimal`, `corporate-blue`, `neon-emerald` + any custom themes generated by `brand`.
...
Support skills: `mira-img-animator` (animates existing images), `mira-chart` (data charts from CSV/JSON, images or hand sketches, with a best-type recommendation), `mira-image-template` (builds a new deck template from screenshots and/or a logo, recognizing the design system and layout, then registers it for `mira-new`) and `mira-brand-template` (generates a theme + deck template from corporate brand identity files — PowerPoint, PDF, CSS, URL, etc.). Entry and helpers: `mira-new`, `mira-references`, `mira-get-videos`.
...
Custom templates generated by `brand` live in `mira-templates/` and are discovered automatically by `mira-new` alongside the built-in ones.
...
npx mira-animator new <name>         # create a new deck from a template
npx mira-animator brand <name>       # generate theme + template from brand assets
...
| `new <name>` | Creates a new deck from a template. Options: `--deck=...` `--theme=...` |
| `brand <name>` | Generates a Mira theme + deck template from brand identity files. Options: `--source=<path-or-url>` |
...
> Creating a deck is also possible conversationally in Claude with `/mira-new` (see [Creating a deck](#creating-a-deck)).

---

## Brand Template Generator

Turn your company's brand assets into a Mira-ready theme + deck template — automatically.

```bash
npx mira-animator brand acme-corp --source=./brand/style-guide.css
```

The `brand` command accepts PowerPoint, PDF, CSS, HTML, CSV, SVG, images, ZIP archives, or website URLs. It extracts colors, typography, logos and layout patterns, then produces:

- `mira-templates/themes/<name>.css` — a complete Mira theme with all 15 `--mira-*` variables
- `mira-templates/decks/<name>/index.html` — a deck template skeleton ready for `mira-new`

The generated template appears automatically in `mira-new`'s template and theme lists.

**Supported formats:** `.pptx`, `.ppt`, `.pdf`, `.css`, `.html`, `.csv`, `.svg`, `.png`, `.jpg`, `.webp`, `.zip`, and URLs.

Also available as an agent skill: `/mira-brand-template` for conversational use with override support.

See [docs/brand-template.md](docs/brand-template.md) (EN) | [docs/brand-template.pt-br.md](docs/brand-template.pt-br.md) (PT-BR) | [docs/brand-template.es.md](docs/brand-template.es.md) (ES).
